...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MNPRcoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MNPRcoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("MNPRcoinMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MNPRcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
