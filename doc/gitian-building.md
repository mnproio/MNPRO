...
*Setup instructions for a gitian build of MNPRO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MNPRO
...
Clone the git repositories for mnpro and gitian and then checkout the mnpro version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mnpro repository
...
To build MNPRO (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mnpro repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mnpro/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/mnpro/mnpro
...
For example:
```bash
URL=https://github.com/konez2k/mnpro.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mnpro=${COMMIT} --url mnpro=${URL} ../mnpro/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mnpro=${COMMIT} --url mnpro=${URL} ../mnpro/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mnpro=${COMMIT} --url mnpro=${URL} ../mnpro/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mnpro-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mnpro-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mnpro-build.assert
```
...
[mnpro/gitian.sigs](https://github.com/mnpro/gitian.sigs/) repository, or if that's not possible to create a pull
request.
